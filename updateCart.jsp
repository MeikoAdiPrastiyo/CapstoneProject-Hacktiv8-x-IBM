<%-- updateCart.jsp --%>
<%@ page language="java" contentType="application/json; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ page import="java.sql.*" %>
<%@ page import="com.google.gson.Gson" %>
<%@ page import="java.util.HashMap" %>
<%@ page import="java.util.Map" %>
<%
    response.setContentType("application/json");
    
    // Create response map
    Map<String, Object> responseMap = new HashMap<>();
    Gson gson = new Gson();
    
    try {
        // Check if user is logged in
        String email = (String) session.getAttribute("email");
        if (email == null) {
            responseMap.put("success", false);
            responseMap.put("message", "Silakan login terlebih dahulu");
            out.print(gson.toJson(responseMap));
            return;
        }
        
        // Get parameters
        String keranjangIdStr = request.getParameter("keranjangId");
        String quantityStr = request.getParameter("quantity");
        
        if (keranjangIdStr == null || quantityStr == null) {
            responseMap.put("success", false);
            responseMap.put("message", "Parameter tidak lengkap");
            out.print(gson.toJson(responseMap));
            return;
        }
        
        int keranjangId = Integer.parseInt(keranjangIdStr);
        int quantity = Integer.parseInt(quantityStr);
        
        if (quantity < 1) {
            responseMap.put("success", false);
            responseMap.put("message", "Jumlah minimal 1");
            out.print(gson.toJson(responseMap));
            return;
        }
        
        // Database connection
        String url = "jdbc:mysql://localhost:3306/webcapstone";
        String user = "root";
        String dbpass = "";
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn = DriverManager.getConnection(url, user, dbpass);
            
            // Get user ID
            String getUserSql = "SELECT id FROM user WHERE email = ?";
            pstmt = conn.prepareStatement(getUserSql);
            pstmt.setString(1, email);
            rs = pstmt.executeQuery();
            
            int userId = 0;
            if (rs.next()) {
                userId = rs.getInt("id");
            } else {
                responseMap.put("success", false);
                responseMap.put("message", "User tidak ditemukan");
                out.print(gson.toJson(responseMap));
                return;
            }
            rs.close();
            pstmt.close();
            
            // Check if cart item belongs to user and get stock info
            String checkSql = "SELECT k.barang_id, b.quantity as stok, b.nama_barang " +
                             "FROM keranjang k " +
                             "JOIN barang b ON k.barang_id = b.id " +
                             "WHERE k.id = ? AND k.user_id = ?";
            pstmt = conn.prepareStatement(checkSql);
            pstmt.setInt(1, keranjangId);
            pstmt.setInt(2, userId);
            rs = pstmt.executeQuery();
            
            if (!rs.next()) {
                responseMap.put("success", false);
                responseMap.put("message", "Item keranjang tidak ditemukan");
                out.print(gson.toJson(responseMap));
                return;
            }
            
            int stok = rs.getInt("stok");
            String namaBarang = rs.getString("nama_barang");
            
            if (quantity > stok) {
                responseMap.put("success", false);
                responseMap.put("message", "Jumlah melebihi stok yang tersedia (" + stok + ")");
                out.print(gson.toJson(responseMap));
                return;
            }
            rs.close();
            pstmt.close();
            
            // Update cart quantity
            String updateSql = "UPDATE keranjang SET quantity = ? WHERE id = ? AND user_id = ?";
            pstmt = conn.prepareStatement(updateSql);
            pstmt.setInt(1, quantity);
            pstmt.setInt(2, keranjangId);
            pstmt.setInt(3, userId);
            
            int rowsAffected = pstmt.executeUpdate();
            
            if (rowsAffected > 0) {
                responseMap.put("success", true);
                responseMap.put("message", "Jumlah " + namaBarang + " berhasil diupdate");
            } else {
                responseMap.put("success", false);
                responseMap.put("message", "Gagal mengupdate jumlah item");
            }
            
        } catch (Exception e) {
            responseMap.put("success", false);
            responseMap.put("message", "Error database: " + e.getMessage());
        } finally {
            if (rs != null) try { rs.close(); } catch (SQLException e) { e.printStackTrace(); }
            if (pstmt != null) try { pstmt.close(); } catch (SQLException e) { e.printStackTrace(); }
            if (conn != null) try { conn.close(); } catch (SQLException e) { e.printStackTrace(); }
        }
        
    } catch (Exception e) {
        responseMap.put("success", false);
        responseMap.put("message", "Error: " + e.getMessage());
    }
    
    out.print(gson.toJson(responseMap));
%>